...
def create_fn(body, spec, patch, **kwargs):
    ...
    # Placement policy when preferred location is specified
    ...
    if 'locations' not in spec:
        mode = 'create'
        fogapp_locations = getFogAppLocations(fogapp_name, fogapp_cpu_request, fogapp_memory_request, fogapp_replicas, clusters_qty, placement_policy, mode)
        ...
        # if none of the fog clusters have the right sized nodes
...
def update_fn(spec, status, namespace, logger, patch, **kwargs):
    ...
    if len(eligible_clusters_sorted) == len(fogapp_current_locations) and fogapp_current_locations == eligible_clusters_sorted:
        ...
        for cluster in eligible_clusters:
            ...
            patchDeployment(cluster, fogapp_name, deployment_body)
            i += 1
        ...
        for cluster in fogapp_current_locations:
            if cluster not in eligible_clusters:
                ...
                deleteDeployment(cluster, fogapp_name)
        for cluster in eligible_clusters:
...
def create_fn(body, spec, meta, patch, **kwargs):
    ...
    if 'locations' not in spec:
        current_clusters, original_clusters = getFogAppClusters(fogapp_name)
    else:
        input_clusters = spec['locations'].split(",")
        current_clusters = []
        for location in input_clusters:
    ...
    return {'fogapp_name': fogapp_name, 'fogapp_locations': current_clusters, 'fogapp_status': 'provisioned'}
...
def update_fn(body, spec, meta, patch, **kwargs):
    ...
    if 'locations' not in spec:
        current_deployment_clusters, original_deployment_clusters = getFogAppClusters(fogapp_name)
    else:
        input_clusters = spec['locations'].split(",")
    ...
    current_service_clusters, original_service_clusters = getServiceClusters(fogapp_name)
    ...
    if current_service_clusters == current_deployment_clusters:
        ...
        for cluster in current_service_clusters:
            if cluster not in current_deployment_clusters:
                ...
                deleteService(cluster, fogapp_name)
    ...
    return {'fogapp_name': fogapp_name, 'fogapp_locations': current_deployment_clusters, 'fogapp_status': 'provisioned'}
...
def delete_fn(body, spec, patch, **kwargs):
    ...
    current_clusters, original_clusters = getServiceClusters(fogapp_name)
